         ...
         for(int i=0; i<blocks.block.length; i++)
            {
               ...
               for(int j=0;j<blocks.block[0].length;j++)
               {
                   ...
                   {
                       Rectangle PaintBlock = new Rectangle((j-1)*blocks.blockwidth+80,(i-1)*blocks.blockheight+50, blocks.blockwidth,blocks.blockheight);
                       
                       
                       
                       Rectangle PaintBall = new Rectangle(ball.kordX,ball.kordY,ball.width,ball.height); //Apibrėžiamas blokas kaip stačiakampis objektas
                       
                       Rectangle PaintedBlock= PaintBlock;  //Apibrėžiamas kamuolys kaip stačiakampis objektas
                       
                       if(PaintBlock.intersects(PaintBall)) //Tikrinama, ar kamuolys susiduria su bloku
                       {
                           BrickAmount--;
                           blocks.Destroyed(i,j);
                           
                           if( ball.kordX+19 <= PaintedBlock.x || ball.kordX+2 >= PaintedBlock.x+PaintedBlock.width)
                           {
                               ball.changemovX();
                           }
                           else
